...
void moveImage(const cv::Mat &src, cv::Mat &dst, const cv::Point2f &translation) {
    ...
    cv::warpAffine(src, dst, translationMatrix, src.size(), cv::INTER_NEAREST, cv::BORDER_CONSTANT, cv::Scalar(255, 255, 255));
}

// 旋转图像的函数（在原点处旋转，使用最近邻插值）
void rotateImage(const cv::Mat &src, cv::Mat &dst, double angle) {
    cv::Mat rot = cv::getRotationMatrix2D(cv::Point(0, 0), angle, 1.0);
    cv::warpAffine(src, dst, rot, dst.size(), cv::INTER_NEAREST, cv::BORDER_CONSTANT, cv::Scalar(255, 255, 255));
}
...
int main() {
    ...
    // 图片文件路径
    std::string filePath = "/home/muhammad/下载/第四次考核/dataset_任务一/20.jpg";
    ...
    // 缩放图像为 (800, 800) 大小，使用最近邻插值
    cv::Mat scaledImage;
    cv::resize(src, scaledImage, cv::Size(800, 800), 0, 0, cv::INTER_NEAREST);
    ...
    // 保存图像
    std::string savePath = "/home/muhammad/图片/考核一/考核二十_rotated_and_moved.jpg";
